package com.sixteencolorgames.DandRSponge;
...
@Plugin(id = "dandrsponge", name = "DandR Sponge", version = "0.1")
public class DandRSponge {

	public static final String PLUGIN_ID = "dandrsponge";
	@Inject
	Logger logger;

	private static final WorldArchetype SKYHELL = WorldArchetype.builder().from(WorldArchetypes.THE_NETHER)
			.generatorModifiers(WorldGeneratorModifiers.SKYLANDS).build("the_skyhell", "The SkyHell");

	@Listener
	public void onGamePreInitialization(GamePreInitializationEvent event) {
		Sponge.getCommandManager().register(this,
				CommandSpec.builder().description(Text.of("Teleports a player to another world"))
						.arguments(seq(playerOrSource(Text.of("target")), onlyOne(world(Text.of("world")))))
						.permission(PLUGIN_ID + ".command.tpworld").executor((src, args) -> {
							final Optional<WorldProperties> optWorldProperties = args.getOne("world");
							final Optional<World> optWorld = Sponge.getServer()
									.getWorld(optWorldProperties.get().getWorldName());
							if (!optWorld.isPresent()) {
								throw new CommandException(Text.of("World [",
										Text.of(TextColors.AQUA, optWorldProperties.get().getWorldName()),
										"] " + "was not found."));
							}
							for (Player target : args.<Player> getAll("target")) {
								target.setLocation(new Location<>(optWorld.get(),
										optWorld.get().getProperties().getSpawnPosition()));
							}
	...
	public void onGameStartingServerEvent(GameStartingServerEvent event) {
		...
		createAndLoadWorld("skyhell", SKYHELL);
	}
	...
	private static class CommandSpawn implements CommandCallable {
					...
					if ("-i".equals(args[0])) {
						...
						if (args.length == 1) {
							...
							if (optWorldCandidate.isPresent()) {
								...
								source.sendMessage(Text.of("World [", TextColors.AQUA, args[1], TextColors.WHITE,
										"] was not found."));
								return CommandResult.success();
							}
						}
						...
						source.sendMessage(Text.of("World [", TextColors.AQUA, world.getName(), TextColors.WHITE,
								"]: spawn -> x [", TextColors.GREEN, spawnCoordinates.getX(), TextColors.WHITE,
								"] | y [", TextColors.GREEN, spawnCoordinates.getY(), TextColors.WHITE, "] | z [",
								TextColors.GREEN, spawnCoordinates.getZ(), TextColors.WHITE, "]."));
						...
						if (args.length == 1) {
							...
							try {
								spawnCoordinates = new Vector3i(Integer.parseInt(args[1]), Integer.parseInt(args[2]),
										Integer.parseInt(args[3]));
							} catch (Exception ex) {
								source.sendMessage(Text.of(
										"Invalid spawn coordinates, must be in numeric format. Ex. /spawn -s 0 64 0."));
								return CommandResult.success();
							}
						}
						...
						source.sendMessage(Text.of("World [", TextColors.AQUA, world.getName(), TextColors.WHITE,
								"]: spawn set to -> x [", TextColors.GREEN, spawnCoordinates.getX(), TextColors.WHITE,
								"] | y [", TextColors.GREEN, spawnCoordinates.getY(), TextColors.WHITE, "] | z [",
								TextColors.GREEN, spawnCoordinates.getZ(), TextColors.WHITE, "]."));
					}
				}
		...
		@Override
		public List<String> getSuggestions(CommandSource source, String arguments, Location<World> pos)
